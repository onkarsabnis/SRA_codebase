...
############ Newly experimenting
        ...
        #                     for fs in [2,3]
        #                     ])
        # # self.fc = nn.Linear(len([2,3]) * 50, output_dim)
########### Newly experimenting
    ...
    def forward(self, inputs):
        ...
        encode_left = self.dropout(
            self._forward_lstm_with_padded_sequence(proj_left, length_left))  ###originally instead of coved_left we had proj_left
        encode_right = self.dropout(
            self._forward_lstm_with_padded_sequence(proj_right, length_right)) ###originally instead of conved_right we had proj_right
...
##### Newly experimenting
        ...
        # embedded_proj_left = proj_left.unsqueeze(1)
        # embedded_proj_right = proj_right.unsqueeze(1)

        # #embedded = [batch size, 1, sent len, emb dim]
        # conved_left = [F.relu(conv(embedded_proj_left)).squeeze(3) for conv in self.convs]
        # conved_right = [F.relu(conv(embedded_proj_right)).squeeze(3) for conv in self.convs]
        # #conved_n = [batch size, n_filters, sent len - filter_sizes[n] + 1]

        # # pooled_left = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in conved_left]
        # # pooled_right = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in conved_right]
